...
public class ModifyContentFrameAction implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("confirm".equals(command)) {
			...
			// 判断用户填写的任务信息是否完整
			if (null != samplesQuantity && !"".equals(samplesQuantity.trim()) && null != singleObjectQuantity && !"".equals(singleObjectQuantity.trim())) {
				// 判断用户填写的是否是数字
				if(StringUtil.isNumeric(samplesQuantity) && StringUtil.isNumeric(singleObjectQuantity)){
					//判断用户填写的数值是否在取值范围之内
					int[] samplesRange = StringUtil.rangeToMinMax(jl_sampleQuantityRange.getText());
					int[] singleRange = StringUtil.rangeToMinMax(jl_singleQuantityRange.getText());
					int samplesQuantityInt = Integer.valueOf(samplesQuantity);
					int singleObjectQuantityInt = Integer.valueOf(singleObjectQuantity);
					if(samplesQuantityInt >= samplesRange[0] && samplesQuantityInt <= samplesRange[1] &&
							singleObjectQuantityInt >= singleRange[0] && singleObjectQuantityInt <= singleRange[1]){
						final int contentRow = jt_inspectionContent.getSelectedRow();
						int contentId = (int) jt_inspectionContent.getValueAt(contentRow, 0);
						//计算检验内容金额
						double oldContentAmount = (double) jt_inspectionContent.getValueAt(contentRow, 5);
						int chargeStandard = (int) jt_inspectionContent.getValueAt(contentRow, 4);
						...
						new SwingWorker<InspectionContent, Void>() {
							protected InspectionContent doInBackground()
									...
									.updateInspectionContent(quoteTask, quoteProject, inspectionBatch, inspectionContent);
							}
							protected void done() {
								try {
									InspectionContent inspectionContent = get();
									if (null != inspectionContent) {
										JOptionPane.showMessageDialog(null, "任务修改成功！", "提示信息", JOptionPane.PLAIN_MESSAGE);
										//更新检验内容UI组件
										jt_inspectionContent.setValueAt(inspectionContent.getSampleQuantity(), contentRow, 2);
										jt_inspectionContent.setValueAt(inspectionContent.getSingleObjectQuantity(), contentRow, 3);
										jt_inspectionContent.setValueAt(inspectionContent.getInspectionContentAmount(), contentRow, 5);
										//更新检验批UI组件
										...
										jaDialog.dispose();
											...
											JOptionPane.showMessageDialog(null, "任务修改失败！", "提示信息", JOptionPane.ERROR_MESSAGE);
									}
						...
						JOptionPane.showMessageDialog(null, "请确保您输入的数字在对应的取值范围之内！", "提示信息", JOptionPane.WARNING_MESSAGE);
					}
				}else{
					JOptionPane.showMessageDialog(null, "请输入大于0的整形数字", "提示信息", JOptionPane.WARNING_MESSAGE);
				}
			}else{
				JOptionPane.showMessageDialog(null, "您修改后的值不能是空值！", "提示信息", JOptionPane.WARNING_MESSAGE);
			}
		} else if ("cancel".equals(command)) {
			int flag = JOptionPane.showConfirmDialog(null, "确定退出么？", "取消修改检验内容", JOptionPane.OK_OPTION);

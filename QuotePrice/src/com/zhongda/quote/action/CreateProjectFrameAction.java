...
public class CreateProjectFrameAction implements ItemListener {
	...
	private void provinceCityCountyLinkage(ItemEvent e, JComboBox<Address> jcb_CityOrCounty) {
		...
		new SwingWorker<List<Address>, Address>(){
			...
			protected List<Address> doInBackground() throws Exception {
				return new AddressServiceImpl().queryAllCityOrCountyByParent(address.getId());
			}
			...
			protected void done() {
				...
				try {
					...
					ComboBoxModel<Address> comboBoxModel = new DefaultComboBoxModel<Address>(model);
